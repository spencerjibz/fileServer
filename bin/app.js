...
program
  ...
  .option(
    "-N,--noBrowser [value]",
    "disable opening the browser,default is false",
  )
  .description(
  ...
  .action(function (options) {
    ...
    var dirName =
      options.dirName === undefined ? process.cwd() : options.dirName;
    ...
    try {
      ...
      process.on("SIGINT" || "SIGTERM", () => {
        db.run("DELETE FROM servers WHERE pid =?", [t.pid], function (err) {
          ...
          console.log(
            this.changes +
              "fileServer instance (s) teminated  and database cleared",
          );
      ...
      db.serialize(function () {
        ...
        if (
          ...
          fs.readFileSync(dbPath, { encoding: "utf8" }).length === 0
        ) {
          db.run(
            "CREATE TABLE servers(Serveraddress TEXT,dir TEXT,pid INTEGER)",
          );
        }

        var stmt = db.prepare(
          "INSERT INTO servers(Serveraddress,dir,pid) VALUES (?,?,?)",
        );
...
program
  ...
  .action(function () {
    ...
    if (
      fs.readFileSync(os.tmpdir + "/test.db", { encoding: "utf8" }).length > 0
    ) {
