      ...
      var directory = function directory(res, path) {
        ...
        fs.readdir(path, function onReaddir(err, list) {
          if (err) {
            ...
            res.end(" <h1> Error response </h1> <br>\n           <p> Error code:" + res.statusCode + "</p> <br>\n           <p> Message:" + err.message.replace(err.code + ':', '') + "</p>\n           \n        </div>");
          } else {
            var dir = decodeURIComponent(stream.path).replace(/\\+|\/+/g, '/');
            // enable browser to open .dir type of folder by adding a lash to end of the folder name
            ...
            res.end("\n         <h1>Directory listing for ".concat(dir, "</h1>\n         <hr> \n         <ul>").concat(results.map(function (v) {
              return "<li> <a href='".concat(window.location.pathname.replace('blank', "".concat(encodeURIComponent(v))), "'>").concat(v, "</a>\n         \n         </li>");
            }).join(' '), "</ul>\n         <hr>\n         \n         "));
          }
        });
      };
      ...
      this.Server = http.createServer(function onRequest(req, res) {
        ...
        logger(req, res, function (err) {
          ...
          send(req, parseUrl(req).pathname, {
            index: false,
            root: this.dirName
          }).on('error', error).once('directory', directory).pipe(res);
        });
      });
