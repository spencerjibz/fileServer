...
function FileServer(dirName, port, noBrowser) {
  ...
  if (typeof this.portName !== "number") {
    return new Error(
      "the port should be a number not ".concat(_typeof(this.port)),
    );
  }
  if (typeof dirName !== "string") {
    return new Error(
      "string parameter is expected for the filepath not ".concat(
        _typeof(dirName),
      ),
    );
  } //validation of port
  // set dirname after type checking
    ...
    fs.access(this.dirName, function (err, done) {
      ...
      this.Server.listen(this.portName, function () {
        log(
          "http server listening at http://127.0.0.1:".concat(
            _this.portName,
            " ",
          ),
        ); /// create a db file int tmpfolder
